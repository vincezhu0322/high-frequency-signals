...
def get_buy_will(cxt):
    ...
    bid_order = trander[['code', 'time_flag', 'bid_id', 'real_bid_volume']].rename(
        columns={'bid_id': 'order_id', 'real_bid_volume': 'volume'})
    bid_order['bs_flag'] = 1
    ask_order = trander[['code', 'time_flag', 'ask_id', 'real_ask_volume']].rename(
        columns={'ask_id': 'order_id', 'real_ask_volume': 'volume'})
    ask_order['bs_flag'] = -1
    order = cudf.concat([order[['code', 'time_flag', 'order_id', 'bs_flag', 'volume']], bid_order, ask_order])
    order = order.sort_values(['code', 'order_id', 'volume'])
    order = order[order.volume != 0].drop_duplicates(subset=['code', 'order_id'], keep='last')
    bid_order = order[order.bs_flag == 1].groupby(['code', 'time_flag']).volume.sum()
    ask_order = order[order.bs_flag == -1].groupby(['code', 'time_flag']).volume.sum()
    order_temp = (bid_order - ask_order).fillna(0)
    trander = trander[trander.trans_type == 1]
    bid_trans = trander[trander.bs_flag == 1].groupby(['code', 'time_flag']).volume.sum().sort_index()
    ask_trans = trander[trander.bs_flag == -1].groupby(['code', 'time_flag']).volume.sum().sort_index()
    trans_temp = (bid_trans - ask_trans).fillna(0)
    res = (trans_temp + order_temp).fillna(0).reset_index().rename(columns={'volume': 'buy_will'})
    return res


pipeline = HftPipeline('buy_will', include_order=True, include_trans_with_order=True)
pipeline.add_block_step(get_buy_will)
pipeline.gen_factors(['buy_will'])
